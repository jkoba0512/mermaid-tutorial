...
Live Editor には主に 3 か所、保存・書き出しのボタンがあります。

- **右上「Share」** — 図の内容を URL に埋め込んで共有できます。
  URL を友人に送ると、相手も同じ図を開いて編集できます。**ログイン不要** で
  使えるので、本チュートリアルではこれが基本の保存方法です。
- **右上「Save diagram」** — 押すとログインを求められ、認証後は **別製品
  「Mermaid Chart」の Visual Editor** に遷移します。AI 機能や視覚編集が
  使えますが、クラウド保存・AI クレジットなどは **有料プラン中心** です。
  本チュートリアルでは使いません。  
  もし誤って遷移してしまったら、ブラウザの戻るボタンか
  <https://mermaid.ai/live/> を再度開けば Live Editor に戻れます。
- **左下「Actions」** — クリックするとメニューが開き、**PNG / SVG** などの
  画像形式でダウンロードできます。Word や PowerPoint に貼り付けたいときに
  便利です。

!!! warning "Share の URL は「URL 自体」が記憶域"
    「Share」で得られる URL は、図の内容をブラウザ上で圧縮して URL に
